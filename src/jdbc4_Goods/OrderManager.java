...
public class OrderManager {
	...
	/*
	 * 1.회원가입 2.로그인 3.내정보 확인
	 * 
	 * 4.상품목록 출력 5.주문 6.주문내역 확인 7.주문취소
	 */
	// 메뉴 출력 기능
	...
	public void showMenu() {
		if (loginId.equals("")) {
			...
			System.out.println("\n[" + loginId + " 회원님]");
			System.out.println("=======================================================");
			System.out.println("1.내정보 | 2.로그아웃 | 3.상품주문 | 4.주문내역 | 5.주문취소 | 0.종료");
			System.out.println("=======================================================");
			System.out.print("메뉴 선택 >> ");
		}
	}
	...
	public void goodsOrder() {
		/*
		 * 현재 판매중인 상품목록 출력 (상품번호, 상품이름) user >> 상품 선택 --ODGNUM 선택한 상품의 상세정보 출력 (상품이름,
		 * 상품가격, 재고) 주문하시겠습니까? (1.예 | 2.아니오) >> user >> (1) > 주문수량입력 -- ODACCOUNT 주문처리
		 * 1. ORDERINFO 테이블에 INSERT (주문코드, 상품번호, 주문수량, 아이디, 주문일) 주문코드 : OD001부터 자동생성 2.
		 * GOODS 테이블에 UPDATE (상품수량) 주문한 상품의 재고를 수정
		 * 
		 */
			...
			if (goodsList.get(i).getGnum() == selGnum) { // 출력된 상품번호 중에서 선택해야 진행
				...
				if (orderConfirm == 1) { // 주문
					...
					if (goodInfo.getGamount() >= orderAmount) { // 주문수량과 재고 비교
						...
						if (insertResult > 0) {
							// GOODS 테이블에 상품수량 수정
							oddao.updateGoodsAmount(goodInfo.getGnum(), orderAmount, "minus");
	...
	public void orderList() {
		...
		if (myOdList.size() <= 0) {
			...
			for (int i = 0; i < myOdList.size(); i++) {
				...
				sum = sum + (myOdList.get(i).getGprice() * myOdList.get(i).getOdamount());
			}
			/*
			 * 총 주문수, 총 주문액
			 */
			System.out.print("총 주문 수 : " + myOdList.size());
			System.out.println(", 총 주문액 : " + sum);
		}
	}
	...
	public void orderCancel() {
		...
		if (myOdList.size() <= 0) {
			...
			if(menuSel >= 0 && menuSel < myOdList.size()) { // 제대로 된 번호 선택 확인
				String odcode = myOdList.get(menuSel).getOdcode();
				System.out.println("선택한 주문코드 : " + odcode);
								
				OrderInfo odInfo = oddao.getOrderInfo(odcode);
				int odGnum = odInfo.getOdgnum();
				int odAmount = odInfo.getOdamount();
				// ORDERINFO 테이블에서 삭제
